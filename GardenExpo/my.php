    ...
    <div class="box">
        ...
        <div class="box-2">
            <div class="box-2-1">
                <div class="box-2-1-1">
                    <span>个人留言列表</span>
                    <a class="wz" title="点击写留言" href="writeMessage.php">写留言</a>
                </div>
                <?php
                    if (isset($_COOKIE['login']) && $_COOKIE['login'] ==1 ) {
                ?>
                <div class="box-2-1-2">
                    ...
                    if (!$link) die("数据库连接错误");
                    $uid = $_COOKIE['uid'];
                    $sql = "SELECT * FROM message WHERE uid={$uid} order by message_time DESC";
                    $result = mysqli_query($link, $sql);
                    if (!$result) die("数据库查询错误".$sql);

                    while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
                        echo "<a href=\"myMessageDetail.php?mid={$row['mid']}\">{$row['mtitle']}<span>{$row['message_time']}</span></a>";
                    }
                    ?>
                </div>
                <?php
            ...
            <div class="box-2-2">
                ...
                <div class="box-2-2-2">
                    <span>分类</span>
                    <a href="my.php" style="width: 100px">查看个人留言</a>
                    <a href="ticketDetail.php">查看电子票</a>
